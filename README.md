...
Запуск полноценного интерфейса управления без аргументов:

```bash
wgmesh
```

В TUI доступны **100% всех возможностей программы**:
- **Топология сети**: ASCII-визуализация цепочек хопов и связей нод.
- **Управление узлами (`b` / `n` / `Del`)**: Zero-Touch SSH Bootstrap, добавление, удаление с поддержкой `protected: true` подтверждений.
- **Маршруты (`r` / `e` / `+` / `-`)**: Добавление, интерактивное редактирование и защита от случайного удаления.
- **Клиенты и Списки (`c` / `l`)**: Настройка устройств клиентов и доменных правил Split Tunneling.
- **Применение по SSH (`a`)**: Выполнение `apply` с реальным логированием шагов.
- **Глубокая диагностика (`d`)**: Запуск `wgmesh doctor` прямо в TUI с цветными индикаторами здоровья.
- **Экспорт & QR (`x`)**: Экспорт профилей (WireGuard, AmneziaWG, Sing-box JSON, URIs) и рендеринг ASCII QR-кода прямо на экране!
- **Git Синхронизация (`g`)**: Просмотр `git status`, создание коммита и push в репозиторий.
